    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings {
                ...
                // ?? Do not interpolate layer indices between vertices
                nointerpolation uint idxA : TEXCOORD3;
                nointerpolation uint idxB : TEXCOORD4;
            ...
            Varyings vert (Attributes v)
            {
                ...
                o.uv       = v.uv * _UVScale;
                // round and pack to integer slices once, then keep them flat
                o.idxA     = (uint)round(saturate(v.uv2.x) * 255.0);
                o.idxB     = (uint)round(saturate(v.uv2.y) * 255.0);
            ...
            float3 SampleLayer(float2 uv, uint layer)
            {
                // overload expects float for slice, cast is fine:
                return SAMPLE_TEXTURE2D_ARRAY(_AlbedoArray, sampler_AlbedoArray, uv, (float)layer).rgb;
            }
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                // sample the two layers, then blend by w
                ...
                // float3 albedo = float3(i.idxB / 8.0, i.wBlend, 0)
